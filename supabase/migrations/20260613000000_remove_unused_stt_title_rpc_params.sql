...
-- STT 상담 제목은 RPC 내부에서 최종 확정하므로 미사용 title 입력
-- 파라미터를 제거한다.
-- =====================================================================

drop function if exists public.create_stt_consultation_with_snapshot(
  uuid,
  text,
  text,
  text,
...
begin
  ...
  into v_sequence
  from consultation
  where consultation.client_id = p_client_id
    and consultation.created_at >= v_day_start
    and consultation.created_at < v_day_end;
