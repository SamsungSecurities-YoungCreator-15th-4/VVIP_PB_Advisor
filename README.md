...
| 외부 데이터 | DART 전자공시(기업 재무) · yfinance(시세) | — |
...
| --- | --- | --- |
| 업로드·실시간 STT | 세후 계산 | 스트레스 테스트 |
| 화자 매핑 | 포트폴리오 비교 (A/B) | RAG · 기업재무 (DART) |
| IPS (RRTTLLU) 조율 | — | 인사이트 (출처 citation) |
...
| --- | --- |
| `GET /health` | 헬스 체크 |
| `POST /auth/logout` | PB 세션 로그아웃 (JWT 인증) |
| `GET · POST /clients` | 담당 고객 목록 조회 · 신규 고객 등록 (pb_id 격리) |
| `POST /consultations/stt` | 상담 오디오 업로드 → STT·화자 매핑 → IPS(RRTTLLU) 추출 → 스냅샷 저장 |
| `WS /consultations/stt/realtime` | 실시간 녹음(브라우저 마이크 PCM) WebSocket STT — `?token=` JWT 인증 |
| `GET /consultations` · `/consultations/detail` | 상담 목록 · 상세 조회 (지난 상담 불러오기) |
| `GET /consultations/initial-ips` | 고객 초기(디폴트) IPS 조회 |
| `POST /portfolio/calculate` · `/portfolio/stress-test` | 세후 포트폴리오 제안(A/B) · 스트레스 테스트 |
| `POST /portfolio/insight` | 중앙 대시보드 스냅샷 요약 인사이트 |
| `POST /rag/insight` | 질의 임베딩 → pgvector 검색 → 답변 + 출처(citation) 반환 |
| `POST /dart/insight` | 기업명 → DART 전자공시 재무 조회 → `gpt-4o` 요약 (수치는 원문 그대로, 접수번호로 감사추적) |
| `POST /tax/insight` | 절세 계산 결과(JSON)를 `gpt-4o`로 요약(temperature=0), 실패 시 템플릿 폴백 |
| `GET /api/macro-indicators` | 실시간 거시지표(금리·환율·지수) — 상단바·PDF 공용 |
...
- **실시간 데이터는 RAG에 임베딩하지 않는다.** 시세 등 변동 데이터는 임베딩 대상이 아니라
  호출 시점에 직접 조회하는 것을 원칙으로 한다. 기업 재무제표 질의(`/dart/insight`)도 코퍼스에
  적재하지 않고 **DART 전자공시에서 실시간 조회**하며, 수치는 원문 그대로 싣고 LLM은 요약만 한다.
