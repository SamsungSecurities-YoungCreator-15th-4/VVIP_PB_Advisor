...
# dev 시드 식별용 고정 UUID(재실행 시 같은 행을 갱신). meta.dev_seed 로 표시해
# 운영 데이터와 구분한다.
DEV_DOCUMENTS = [
    {
        ...
        "chunks": [
            (
                "금융소득 종합과세는 이자소득과 배당소득을 합한 금융소득이 연 2,000만원을 "
                "초과하는 경우, 그 초과분을 다른 종합소득과 합산해 누진세율로 과세하는 제도다. "
                "연 2,000만원 이하의 금융소득은 15.4%(지방소득세 포함) 원천징수로 분리과세 "
                "효과가 있다. (소득세법 제14조·제62조)"
            ),
            (
                "금융소득이 연 2,000만원을 초과하면 초과분은 종합소득에 합산되어 누진세율이 "
                "적용되며, 지방소득세를 포함한 최고세율은 49.5%에 이른다. 종합과세 시에도 "
                "원천징수세액과 비교해 큰 금액으로 과세하는 비교과세 방식이 적용된다."
            ),
        ],
    },
    {
        ...
        "meta": {
            "dev_seed": True,
            "published_date": "2026-01-01",
            "source_note": "국세청 2026 세금절약가이드 1권 / 조세특례제한법 §88의2",
        },
        "chunks": [
            (
                "개인종합자산관리계좌(ISA)는 계좌 내 운용수익에 대해 일반형 200만원, 서민형 "
                "400만원까지 비과세하고, 비과세 한도 초과분은 9.9%(지방소득세 포함)로 저율 "
                "분리과세한다. (조세특례제한법 제88조의2)"
            ),
            (
                "ISA는 의무가입기간이 3년이라 중도 해지 시 세제 혜택이 제한될 수 있다. "
                "따라서 자금의 유동성 계획과 함께 가입 여부를 검토해야 한다."
            ),
        ],
    },
    {
        ...
        "meta": {
            "dev_seed": True,
            "published_date": "2026-01-01",
            "source_note": "국세청 2026 세금가이드 2권 / 소득세법 양도소득(국외주식)",
        },
        "chunks": [
            (
                "해외주식 양도소득세는 양도차익에서 연 250만원 기본공제를 적용한 뒤 "
                "22%(국세 20% + 지방소득세 2%) 세율로 과세된다. 양도한 해의 다음 해 5월에 "
                "양도소득세 확정신고를 해야 한다."
            ),
            (
                "같은 해에 실현한 해외주식 양도차익과 양도차손은 서로 통산할 수 있다. 일반 "
                "개인 투자자는 국내 상장주식이나 해외 파생상품의 손익과는 통산할 수 없으므로, "
                "해외주식 양도손익끼리만 상계된다."
            ),
        ],
    },
...
def main() -> None:
    ...
    for doc in DEV_DOCUMENTS:
        ...
        for idx, content in enumerate(doc["chunks"]):
            # embed_query 는 단일 텍스트 임베딩 함수라 청크 임베딩에도 그대로 쓴다.
