...
- 임베딩: OpenAI text-embedding-3-small (1536차원, document_chunk.embedding 차원과 일치)
...
def get_openai_client() -> OpenAI:
    if not os.getenv("OPENAI_API_KEY"):
        raise RuntimeError(
            ...
            "backend/.env 에 키를 추가하세요 (.env.example 참고)."
        )
    return OpenAI()
...
def embed_query(text: str) -> list[float]:
    """질의 텍스트를 text-embedding-3-small 로 임베딩해 1536차원 벡터를 반환한다."""
