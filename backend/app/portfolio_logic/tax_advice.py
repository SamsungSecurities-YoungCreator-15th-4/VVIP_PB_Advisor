...
검증: docs/절세-시뮬레이터-검증정리.md (verify_tax_strategies / verify_tax_overlap / verify_persona_gating)
"""
...
def calc_tax_advice(
    ...
    *,
    ...
    elif not isa_opened and horizon_years is not None and horizon_years < ISA_MANDATORY_HOLDING_YEARS:
        isa_reason = f"투자기간 {horizon_years:g}년 < ISA 의무보유기간 {ISA_MANDATORY_HOLDING_YEARS}년"
    ...
    pension_headroom_won = max(PENSION_TAX_CREDIT_LIMIT_WON - max(pension_used_manwon, 0.0) * 10_000, 0.0)
    ...
    cards.append({
        ...
        "transferableManwon": _won_to_manwon(min(pension_headroom_won, investable_won)) if pension_applicable else 0,
    ...
    bond_saving_won = bond_eligible_won * max(marginal_income_tax_rate - LONG_BOND_SEPARATE_TAX_RATE, 0.0)
    cards.append({
...
def calc_combined_tax_saving(
    ...
    **kwargs,
    ...
    income_by_asset = _income_won_by_asset(portfolio, gross_return, total_won)
    excess = max(sum(income_by_asset.values()) + other_income_won - COMPREHENSIVE_TAX_THRESHOLD, 0.0)
    ...
    if isa["ineligibleReason"]:
        ...
        if income_asset_value > 0:
            ...
            isa_headroom = max(ISA_ANNUAL_LIMIT_WON - kwargs.get("isa_used_manwon", 0.0) * 10_000, 0.0)
            investable = max(total_won - max(kwargs.get("near_term_need_manwon", 0.0), 0.0) * 10_000, 0.0)
